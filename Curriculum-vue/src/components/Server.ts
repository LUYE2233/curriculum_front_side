...
import type { OccupationSum } from '@/classes/OccupationSum'
  ...
  routes() {
    ...
    this.post('/changeOccupation', (schema, req) => {
      ...
      if (data.libId != null && data.timeIndex != null && data.newOccupation != null) {
        ...
        if (data.libId[0] == data.libId[1]) {
          ...
          //深度克隆lib
          const tempLib: Lib = _.cloneDeep(lib)
          ...
          const newTempLib: Lib = _.cloneDeep(newLib)
          //尝试添加新课时
    ...
    this.post('/addOccupation', (schema, req) => {
      ...
      if (data.libId != null && data.newOccupation != null) {
        ...
        //尝试添加课时
        const courseResult: { result: boolean; reason: string } = _.cloneDeep(course).addTime(
