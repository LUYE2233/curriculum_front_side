...
import { OccupationSum } from '@/classes/OccupationSum'
...
const Server = server
//数据准备
let myCourses = ref(new Array<Course>())
let libs = ref(new Array<Lib>())
let user: User
let occupationsSum: Ref<OccupationSum[]> = ref(new Array<OccupationSum>())
const options = [
  { text: '星期一', value: 1 },
  { text: '星期二', value: 2 },
  { text: '星期三', value: 3 },
...
let appendLibId: Ref<[number, number]>
  ...
  await axios({
    ...
    .then((response) => {
      if (response.data.result != false) {
        ...
        newCourse = ref(new Course(user, '', 0, 0))
        load = true
      }
    })
    .catch((error) => {
...
async function loadCourses(): Promise<boolean> {
  ...
  await axios({
    method: 'GET',
    url: 'https://my.api.com/getMyCourses'
  })
    .then((res) => {
      const data: { result: null | Course[]; reason: string } = res.data
      if (data.result != null) {
        myCourses.value.splice(0, Infinity)
        myCourses.value = myCourses.value.concat(data.result)
        loaded = true
      } else {
...
async function loadLibs(): Promise<boolean> {
  let loaded = false
  await axios({
    method: 'GET',
...
async function loadDatas() {
  ...
  let load1 = false
  let load2 = false
  await loadCourses().then((result) => {
    load1 = result
  })
  await loadLibs().then((result) => {
    load2 = result
  })
  if (load1 && load2) {
    occupationsSum.value.splice(0, Infinity)
...
function initAdd(sum: OccupationSum) {
  if (!init) {
    ...
    appendLibId = ref([libs.value[0]._libId, libs.value[0]._libId])
    ...
    appendLibId.value = [libs.value[0]._libId, libs.value[0]._libId]
...
<template>
  ...
  <div v-if="loadDone">
    <el-collapse accordion>
      <div v-for="(sum, index) of occupationsSum" :key="index">
  ...
  <!-- 添加课时 -->
  <el-dialog v-model="addTimeFormVisible" title="添加新课时" width="70%">
